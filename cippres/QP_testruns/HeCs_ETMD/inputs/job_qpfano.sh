...
#PBS -m ae
#PBS -l nodes=node05:ppn=15    #1-16 cpu
#PBS -l mem=60gb

export OMP_NUM_THREADS=15
export QP_ROOT=/mnt/nfs/home/common/quantum_package

EZFIO_NAME=HeCs.ezfio
EXECUTABLE_NAME=cippres_fano

output=${EXECUTABLE_NAME}_${EZFIO_NAME}.out
...
for i in {2..189}
do
 rm HeCs.ezfio/cippres/cfano_cippres.gz
 rm HeCs.ezfio/cippres/efano_cippres.gz
 qp set cippres ici2 $i
 qp get cippres ici2
 qp run cippres_fano
 mv HeCs.ezfio/cippres/cfano_cippres.gz HeCs.ezfio/cippres/cfano_cippres${j}.gz
 mv HeCs.ezfio/cippres/efano_cippres.gz HeCs.ezfio/cippres/efano_cippres${j}.gz
 ((j++))
done

qp run ${EXECUTABLE_NAME} | tee $output
qp unset_file
}
