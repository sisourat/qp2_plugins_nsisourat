...
#PBS -m ae
#PBS -l nodes=node05:ppn=16    #1-16 cpu
#PBS -l mem=60gb
...
for i in {2..71}
do
 rm Ne.ezfio/cippres/cfano_cippres.gz
 rm Ne.ezfio/cippres/efano_cippres.gz
 qp set cippres ici2 $i
 qp get cippres ici2
 qp run cippres_fano
 mv Ne.ezfio/cippres/cfano_cippres.gz Ne.ezfio/cippres/cfano_cippres${j}.gz
 mv Ne.ezfio/cippres/efano_cippres.gz Ne.ezfio/cippres/efano_cippres${j}.gz
 ((j++))
done
