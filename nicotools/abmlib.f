...
C *                  PsiDim*(IntOrder+1).                              *
C *   CData:         Complex*16 data needed in the Func (and possibly  *
C *                  CalcError) routine.                               *
C *   RData:         Real*8 data needed in the Func (and possibly      *
C *                  CalcError) routine.                               *
C *   IData:         Integer data needed in the Func (and possibly     *
C *                  CalcError) routine.                               *
C *   LData:         Logical data needed in the Func (and possibly     *
C *                  CalcError) routine.                               *
C *                                                                    *
C * External routines:                                                 *
C *   Func:            Computes the time derivative DtPsi of Psi at    *
C *                    time AbsTime. Called as                         *
C *       NICO  func(AbsTime,Psi,DtPsi,PsiDim,CData,RData,IData,LData).*
C *   CalcError:     Determines the error of the current integration   *
C *                  step. Called as CalcError(Predicted_Psi,          *
C *                  Corrected_Psi,PsiDim,Error,CData,RData,IData,     *
C *                  LData).                                           *
...
C **********************************************************************
     ...
     +                ErrorCode,AuxPsi,Func,CalcError,CData,
     +                RData,IData,LData)
     ...
     +           MaxOrder = 8)
      ...
      Logical    RestartABM,LData(*)
      Integer    PsiDim,IntOrder,Steps,RepeatedSteps,ErrorCode,IData(*)
      Real*8     IntPeriod,AbsTime,InitStep,TolError,RData(*)
      Complex*16 Psi(PsiDim),DtPsi(PsiDim),AuxPsi(PsiDim,IntOrder+1),
     +           CData(*)
      External   Func,CalcError

      Logical    StepIsRepeated,CurOrdEqIntOrd,UseOldH1
...
C --- EVALUATE FUNCTION WITH PREDICTED PSI ---

      NextTime = Time+H(1)
      Call Func(NextTime,Psi,DtPsi,PsiDim,CData,RData,IData,LData)
...
C --- CALCULATE ERROR AND (IF REQUIRED) NEW STEP SIZE ---
      ...
      Call CalcError(DtPsi,Psi,PsiDim,Error,CData,RData,IData,LData)
...
CNICO     Call WriteStep(Steps,CurrentOrder,H(1),Error,Time)
...
C *   Cdata:     Complex data array (not used in this error routine)   *
C *                                                                    *
C * Output parameters:                                                 *
...
C * Other parameters:                                                  *
C *   CData:     Complex data array (not used in this error routine).  *
C *   RData:     Real data array (not used in this error routine).     *
C *   IData:     Integer data array (not used in this error routine).  *
C *   LData:     Logical data array (not used in this error routine).  *
C *                                                                    *
C * V6.0 MB                                                            *
C * V7.0 GW addition of Data arrays                                    *
C *                                                                    *
C **********************************************************************

      Subroutine AbsABMError (Predicted,Corrected,Psidim,Error,CData,
     +                        RData,IData,LData)

      Implicit None

      Logical    LData(*)
      Integer    PsiDim,IData(*)
      Real*8     Error,RData(*)
      Complex*16 Predicted(PsiDim),Corrected(PsiDim),CData(*)
...
C **********************************************************************
      ...
      Subroutine RelABMError (Predicted,Corrected,Psidim,Error,CData,
     +                        RData,IData,LData)
