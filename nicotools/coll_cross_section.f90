...
double precision :: norm, sig_ion, sig_si, sig_di, sig_exc, sip, dip, stopping
...
sig_di = 0d0
do i = 2, nsta
   stopping = stopping + 27.21d0*(esta(i)-esta(1))*sig(i)
 if(esta(i)<sip) then
...
! write(*,*)'Ionization = ', sig_ion/3.57d0
 write(*,'(a,3(f10.5,1X))')'Exc, SI, DI, Stopping = ', sig_exc/3.57d0, sig_si/3.57d0, sig_di/3.57d0, stopping/3.57d0
